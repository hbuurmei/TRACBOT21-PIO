//switching to timed turns for all turns. Trying to use ultrasonic for orientation turn (find same wall, timed turn)
//Setting up only for Course A (better beacon readings)
//Hopefully run imu integration on hardware timer for orientation turn, then switch to software timer for forward controller (jk didn't work at all)
// CONFIGURATION
...
//turn timings
#define turn_360_left_2_5pi 4250.
#define turn_360_left_2pi 5300.
#define turn_360_right_2_5pi 4150.
#define turn_90_right 1500./2
#define turn_90_left 1800.
...
void searching();
void exit_starting_zone();
void gonna_hit_contact_zone();
...
void start() {
    ...
    if (DO_TEST){
        ...
        state = searching;
        time_state_change = millis();
        if (DEBUG_GENERAL) {Serial.println("entering searching");}
    } else{
        Serial.println("IN START");
        state = pause;
        state_after_pause = exit_starting_zone;
        time_state_change = millis();
    }
...
/*
ORIENTATION USING ULTRASONIC
*/

float distance;
float min_distance = 1000;  //initialize high
unsigned long target_time = 0;
int num_loops = 0;
//note: have to reupload (or at least turn battery off and back on) for this to work properly
void searching(){
    /*
    - 360 degree sweep, save min ultrasonic value, then look for that value again in orienting
    */
   
    if (millis()-time_state_change >= turn_360_left_2pi-1.6*num_loops){ 
        //time includes correction factor to account for delays in US.update(). 1.8 works perfectly for 2.5*PI turn speed, 1.6 good for 2*PI
        stop();
        // turn_left(MIDDLE);
        state = orienting;
        time_state_change = millis();
        ...
        if (distance < min_distance){
            ...
            target_time = millis() - time_state_change; //time it took to turn to min_distance
...
#define version 0   //0: use time, 1: use distance
float US_tolerance = 0.05;  //5 percent
//NOTE: could change to measure time of the turn and repeat the turn that way instead. check if that would work better.
void orienting(){
    /*
    - Find min ultrasonic value to face closest wall, have known orientation to lead into a timed turn
    */
//    Serial.print("check min_distance:  ");
//    Serial.println(min_distance);
//    static float distance = 1000;    //FLAG check this logic (ideally set distance to high value once first run)
//    if (millis() - min_distance <= US_tolerance*min_distance){ //FLAG check if min_distance saves from previous state
    switch (version) {
        case 0:
            if(millis()-time_state_change >= 2000){ //pause 2 seconds
                Serial.println("starting turn");
                turn_left(MIDDLE, 2.2*PI);
                static long time_rename = millis();

                if (millis() - time_rename >= target_time){
                    stop();
                    imu.reset_integrators();
                    time_state_change = millis();
                    Serial.println("FOUND IT");
                    state = exit_starting_zone;
                }
            }
            break;
        case 1:
            //test using distance instead of time
            break;
    }
    
    
} //should now have back of robot facing the nearest wall (current plan is for side wall)
...
void turning_to_gap(){
    if (millis()-time_state_change >= 1200){
        stop();
...
//note: most of these states are named very stupidly bc it is almost 5 AM (many state names shifted earlier than they should be by 1)
void driving_through_gap(){
    if (millis()-time_state_change >= turn_90_right){
        stop();
        imu.reset_integrators();
        // forward();
        // forward_controller = 1;
...
void driving_to_contact_zone(){
    if(millis()-time_state_change>=turn_90_left){
        stop();
        imu.reset_integrators();
        forward();
        forward_controller = 1;
        state = gonna_hit_contact_zone;
        time_state_change = millis();
    }
}
void gonna_hit_contact_zone(){
...
// */
// void turning_to_gap() {
//     if (millis()-time_state_change >= 1800){
//         stop();
//         time_state_change = millis();
//         state = pause;
...
void pause(){
    Serial.println(millis()-time_state_change);
    if(millis()-time_state_change >= 2000){
        forward();
        forward_controller = 1;
        imu.reset_integrators();
